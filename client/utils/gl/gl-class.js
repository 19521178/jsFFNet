 ...
 * =============================================================================
 */
// import {PASSTHROUGH_VERTEX_SHADER, MASK_SHADER, RENDERTENSOR_SHADER, BACKGROUND_SHADER} from "./utils/gl/gl-shaders.js";
// import {createProgram, createTextureFrameBuffer} from "./utils/gl/gl-util.js";
...
class GlShaderProcessor {
    ...
    startProcessFrame(width, height) {
      const gl = this.gl;
      if (!this.frame || width !== this.frame.width || height !== this.frame.height) {
        this.frame = createTextureFrameBuffer(gl, gl.LINEAR, width, height, this.frame);
      }
...
class RenderTensor {
    constructor(gl) {
      this.proc = new GlShaderProcessor(gl, RENDERTENSOR_SHADER);
    }
    process(data) {
      this.proc.startProcessFrame(data.width, data.height);
      this.proc.bindTextures([['frame', data]]);
      return this.proc.finalizeProcessFrame();
    }
  }

class BackgroundMixerRenderTensor {
    constructor(gl) {
      this.proc = new GlShaderProcessor(gl, BACKGROUND_SHADER);
    }
    process(data, background) {
      this.proc.startProcessFrame(data.width, data.height);
      this.proc.bindTextures([['frame', data],['back', background]]);
      return this.proc.finalizeProcessFrame();
    }
  }