    ...
    async function startEncoding(callback) {
        ...
        renderer.addEventListener("message", ({ data }) => {
            if (typeof data === "string" && data === "finish") {
                finalize(callback);
            } else if (typeof data === "string" && data === "delay"){
        ...
        // let idBitmap = 0;
        // let upBlobInterval = setInterval(()=>{
        //     if (idBitmap>=listNameImg.length){
        //         clearInterval(upBlobInterval);
        //     }
        //     ((idBitmap)=>{
        //         ldb.get(listNameImg[idBitmap], (blob)=>{
