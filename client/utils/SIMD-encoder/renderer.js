  ...
  return {
    ...
    read() {
      ...
      if (direct && useGL) {
        ...
        let input;
        if (usingBitmap) {
          // console.log('WW-IF USING BITMAP');
...
async function start(settings, config) {
  ...
  const render = ((canvas, context)=>{
    const next = (idRender)=>{
      ...
      createImageBitmap(blob).then((bitmap)=>context.drawImage(bitmap, 0, 0, canvas.width, canvas.height));
      return true;
    };
    return {next};
  })(canvas, context);
  ...
  function renderFrame() {
    ...
    try {
      if (render.next(idFrame)){
        delete dictBlob[idFrame.toString()];
        idFrame++;
      }
      else{
